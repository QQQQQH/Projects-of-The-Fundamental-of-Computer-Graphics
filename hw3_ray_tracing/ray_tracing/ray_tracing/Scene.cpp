...
glm::vec3 Scene::shade(const Object& object, const glm::vec3& pos, const glm::vec3& norm, const Ray& ray) {
	glm::vec3 ambient = ambientColor * object.ambient(pos);
	...
	glm::vec3 diffuse = diff * diffuseColor * object.diffuse(pos);

	glm::vec3 reflectDir = glm::reflect(-lightDir, norm);
	float spec = glm::pow(max(glm::dot(-ray.dir, reflectDir), 0.0f), object.shininess());
	glm::vec3 specular = specularStrength * spec * object.specular(pos);
...
glm::vec3 Scene::trace_ray(const Ray& ray, unsigned int recursionStep) {
	...
	// 光照强度的第一部分：局部光照强度
	//if (collidedObject->faces.size() == 0) {
	//	cout << collidedObject->kShade() << ", " << collidedObject->kReflect() << ", " << collidedObject->kRefract() << endl;
	//}
